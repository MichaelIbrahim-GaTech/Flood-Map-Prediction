...
class BaseLineModel:
    ...
    def train_flood_prediction_model(self) -> List[CatBoostClassifier]:
        ...
        -------
        CatBoostClassifier
            The trained CatBoostClassifier model.
        """
        models = []
        folds = 5
        ...
        print('Brier score' , bscores)
        
        print(self.selected_features)
        #print(self.X_train[:,self.selected_features].shape)
        self.model = models
...
#        print(self.y_dim_FR)
    ...
    def check_load(self, 
        ...
        self.train_flood_prediction_model()
    ...
    def simple_val(self, 
        ...
        if mode == "roc":
            ...
            y_pred_proba_val = np.zeros(shape=(1,X.shape[0]))
            for m in self.model:
                y_pred_proba_val = y_pred_proba_val + m.predict_proba(X)[:, 1]/len(self.model)
            
            print(y_pred_proba_val.shape)
            score = roc_auc_score(y, y_pred_proba_val.T)
        elif mode == "AP":
            #y_pred_proba_val = self.model.predict_proba(X)[:, 1]
            y_pred_proba_val = np.zeros(shape=(1,X.shape[0]))
            for m in self.model:
            ...
            score = average_precision_score(y, y_pred_proba_val.T)
        else:
    ...
    def predict_prob(self, 
        ...
        for m in self.model:
            y_pred_proba_test = y_pred_proba_test + m.predict_proba(X)[:, 1]/len(self.model)
    ...
    def compute_metric_val(self, y_pred_val, y, mode = "roc", rejected_predictions = []):
        print(mode)
        if mode == "acc":
    ...
    def verb_name(self, 
        ...
        indices : list
            The indices of selected features.
        n_estimators : int
            The number of estimators for the CatBoostClassifier.
        max_depth : int
            The maximum depth for the CatBoostClassifier.
        """
